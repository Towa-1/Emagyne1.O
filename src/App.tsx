  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen p-3 md:p-8 flex flex-col items-center">
        <header className="w-full max-w-6xl flex justify-between items-center mb-6 md:mb-8">
          <div className="flex items-center gap-2 md:gap-3">
            <div className="w-10 h-10 md:w-12 md:h-12 bg-yellow-500 rounded-xl flex items-center justify-center shadow-lg shadow-yellow-500/20">
              <Trophy className="text-slate-950" size={20} />
            </div>
            <h1 className="text-2xl md:text-4xl gold-gradient-text uppercase">Emagyne</h1>
          </div>
          
          {state.phase === 'QUIZ' && (
            <div className="flex items-center gap-3 md:gap-6 glass-panel px-3 md:px-6 py-2 md:py-3 rounded-2xl">
              <div className="flex items-center gap-1 md:gap-2 text-yellow-500 font-mono text-base md:text-xl font-bold">
                <Clock size={20} />
                {formatTime(state.timeRemaining)}
              </div>
              <div className="h-4 md:h-6 w-px bg-yellow-500/20" />
              <button 
              ...
              <button 
                ...
                className="md:hidden text-yellow-500 hover:text-yellow-400 transition-colors ml-1"
          ...
          <AnimatePresence mode="wait">
            ...
            {state.phase === 'QUIZ' && (
              ...
              >
                ...
                <aside className="hidden md:block w-80 glass-panel p-6 rounded-3xl h-fit sticky top-8">
                  <NavigatorContent 
                    state={state} 
                  ...
                  {isMobileNavOpen && (
                    <div className="fixed inset-0 z-50 md:hidden flex items-end justify-center">
                      ...
                      <motion.div
                        ...
                        className="glass-panel relative w-full max-h-[90vh] p-6 rounded-t-3xl overflow-y-auto custom-scrollbar"
                      >
                        <div className="flex justify-between items-center mb-6">
                          <h3 className="text-xl font-black text-yellow-500 uppercase tracking-wider">Question Navigator</h3>
                          <button 
                            onClick={() => setIsMobileNavOpen(false)}
                ...
                <div className="flex-1 flex flex-col gap-4 md:gap-6">
                  <div className="glass-panel p-5 md:p-8 rounded-2xl md:rounded-3xl flex-1 relative overflow-hidden">
                    {state.isPaused && (
                    ...
                    <div className="flex justify-between items-start mb-6 md:mb-8">
                      <span className="px-3 py-1 bg-yellow-500/10 text-yellow-500 text-[10px] md:text-xs font-black rounded-full uppercase tracking-widest">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer group">
                        ...
                        />
                        <div className={cn(
                          "w-4 h-4 md:w-5 md:h-5 rounded border-2 transition-all flex items-center justify-center",
                        ...
                        <span className="text-[10px] md:text-xs font-bold text-slate-500 group-hover:text-slate-300">Mark for Review</span>
                      </label>
                    </div>
                      ...
                      {currentQuestion.type === 'MCQ' ? (
                        currentQuestion.options?.map((option, idx) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "w-full p-4 md:p-5 rounded-xl md:rounded-2xl border text-left transition-all flex items-center gap-3 md:gap-4 group",
                            ...
                            >
                              <div className={cn(
                                "w-7 h-7 md:w-8 md:h-8 rounded-lg flex items-center justify-center text-xs md:text-sm font-black transition-colors shrink-0",
                        ...
                        <div className="flex items-center gap-3 md:gap-4">
                          <input
                            ...
                            className="flex-1 bg-slate-950/50 border border-yellow-500/20 rounded-xl md:rounded-2xl p-4 md:p-5 text-xl md:text-2xl font-black text-yellow-500 focus:outline-none focus:border-yellow-500 transition-colors"
                          />
                          {currentQuestion.unit && (
                            <span className="text-xl md:text-2xl font-black text-slate-500">{currentQuestion.unit}</span>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-3 md:gap-4 mt-4 md:mt-6">
                    <button
                      ...
                      className="w-full sm:w-auto px-5 md:px-6 py-3 md:py-4 rounded-xl md:rounded-2xl border border-yellow-500/20 text-yellow-500 font-bold hover:bg-yellow-500/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 text-sm md:text-base"
                    >
                      <ArrowLeft size={20} />
                      PREVIOUS
                    </button>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full sm:w-auto">
                      {!state.checkedAnswers.has(currentQuestion.id) && (
                        <button
                          ...
                          className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl bg-slate-800 text-yellow-500 font-black hover:bg-slate-700 disabled:opacity-50 transition-all text-sm md:text-base"
                        >
                          CHECK ANSWER
                        </button>
                      )}
                      ...
                      {currentQuestionIndex === state.questions.length - 1 ? (
                        <button
                          onClick={finishExam}
                          className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 rounded-xl md:rounded-2xl bg-yellow-500 text-slate-950 font-black hover:bg-yellow-600 transition-all flex items-center justify-center gap-2 text-sm md:text-base"
                        >
                          FINISH EXAM
